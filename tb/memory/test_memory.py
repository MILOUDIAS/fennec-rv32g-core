...
async def memory_data_test(dut):
    ...
    for address, data in test_data:
        ...
        ), f"Error at address {address}: expected {hex(data)}, got {hex(dut.read_data.value)}"
    ...
    for i in range(40, 4):
        ...
        expected_value = i
        assert (
            dut.read_data.value == expected_value
        ), f"Expected {expected_value}, got {dut.read_data.value} at address {i}"
